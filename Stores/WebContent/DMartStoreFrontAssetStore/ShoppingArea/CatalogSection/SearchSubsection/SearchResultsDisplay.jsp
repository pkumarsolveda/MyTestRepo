...
<html xmlns:wairole="http://www.w3.org/2005/01/wai-rdf/GUIRoleTaxonomy#"
	...
	<head>
		...
		<fmt:message var="metaDescription" key="SEO_DESCRIPTION" bundle="${storeText}"/>
		<fmt:message var="metaKeyword" key="SEO_KEYWORDS" bundle="${storeText}"/>
	...
	<c:when test="${totalContentCount == 0 && totalCount == 1 && !searchMissed && empty WCParam.categoryId && empty WCParam.manufacturer}">
		...
		<c:choose>
			...
			<c:when test="${!empty searchParentCategoryId && !empty searchTopCategoryId}">
				...
				<c:set var="patternName" value="DmartProductURLWithParentAndTopCategory"/>
			</c:when>
			<c:when test="${!empty searchParentCategoryId}">
				...
				<c:set var="patternName" value="DmartProductURLWithParentCategory"/>
			</c:when>
			...
			<c:when test="${WCParam.top == 'Y'}">
				...
				<c:set var="top_category" value="${searchTopCategoryId}" />
				<c:set var="categoryId" value="${WCParam.categoryId}" />
				<c:set var="patternName" value="DmartProductURLWithCategory"/>
			</c:when>
			...
			<c:otherwise>
				...
				<c:set var="patternName" value="DmartProductURL"/>
			</c:otherwise>
		</c:choose>
		...
		<c:if test="${!empty catEntryDisplayUrl}">
			<script type="text/javascript">
					window.location.href = '${catEntryDisplayUrl}';
			</script>
		</c:if>
	</c:when>
	<c:otherwise>
		...
		<wcf:url var="CategoryDisplayURL" patternName="DmartCanonicalCategoryURL" value="Category3">
			<wcf:param name="langId" value="${langId}" />
			<wcf:param name="storeId" value="${storeId}" />
			<wcf:param name="catalogId" value="${catalogId}" />
			<wcf:param name="categoryId" value="${WCParam.categoryId}" />	
					...
					<c:when test="${empty redirect}">
						<script type="text/javascript">
							dojo.addOnLoad(function() { 
								...
								document.getElementById("decodeIt").innerHTML=searchTerm
								searchTerm=decodeURIComponent(document.getElementById("decodeIt").innerText); 								
								dojo.byId("SimpleSearchForm_SearchTerm").value = searchTerm;
								WCParamJS.searchTerm = searchTerm;
							});
						</script>
