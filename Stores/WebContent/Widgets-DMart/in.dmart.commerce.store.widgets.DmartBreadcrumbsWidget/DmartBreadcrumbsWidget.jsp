...
<%-- BEGIN DmartBreadcrumbsWidget.jsp --%>
...
<c:choose>
	<c:when test="${pageName eq 'AdvancedSearchPage' || pageName eq 'StaticSearchPage' || (!empty productId && (empty parent_category_rn && empty categoryId))}">
		<%out.flush();%>
			<c:import url="/Widgets-DMart/in.dmart.commerce.store.widgets.DmartBreadcrumbsWidget/DmartBreadcrumbTrailGeneric.jsp" />
		<%out.flush();%>
	</c:when>
	<c:otherwise>
		<%out.flush();%>
			<c:import url="/Widgets-DMart/in.dmart.commerce.store.widgets.DmartBreadcrumbsWidget/DmartBreadcrumbTrailHierarchy.jsp" />
		<%out.flush();%>
	</c:otherwise>
</c:choose>
...
<%-- END DmartBreadcrumbsWidget.jsp --%>
