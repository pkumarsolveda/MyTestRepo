...
<wcf:useBean var="orderedItemAvailabilities" classname="java.util.ArrayList"/>
<c:forEach var="sku" items="${entitledItems}" varStatus="skuStatus">
	<c:forEach var="itemAvailability" items="${inventoryAvailabilities.InventoryAvailability}" varStatus="status">
		<c:if test="${itemAvailability.productId eq sku.uniqueID}">
			<wcf:set target="${orderedItemAvailabilities}" value="${itemAvailability}" />
		</c:if>
	</c:forEach>
</c:forEach>
...
"onlineInventory": 
	{
	<c:forEach var="itemAvailability" items="${orderedItemAvailabilities}" varStatus="status">
