 ...
 */
	require(["dijit/registry","dojo/date/stamp", "dojo/topic", "dojo/date"]);
	BuyerApprovalListJS = {
		...
		*/
		approveBuyer: function(aprvstatus_id) {
			service = wc.service.getServiceById('AjaxApproveBuyerRequest');
			...
			cursor_wait();
			wc.service.invoke('AjaxApproveBuyerRequest',params);		
		},
		...
		*/
		rejectBuyer: function(aprvstatus_id) {
			service = wc.service.getServiceById('AjaxRejectBuyerRequest');
			...
			cursor_wait();
			wc.service.invoke('AjaxRejectBuyerRequest',params);
		},
		...
		*/
		showResultsPage:function(data){
			...
			wc.render.updateContext('BuyerApprovalTable_Context', {"beginIndex": beginIndex});
		...
		doSearch:function(formId){
			...
			this.updateContext({"beginIndex":"0", "approvalId": form.approvalId.value.replace(/^\s+|\s+$/g, ''), "firstName":form.submitterFirstName.value.replace(/^\s+|\s+$/g, '') , "lastName":form.submitterLastName.value.replace(/^\s+|\s+$/g, ''),"startDate":startDateValue,"endDate":endDateValue});
		},
		...
		updateContext:function(context){
			...
			cursor_wait();
			wc.render.updateContext("BuyerApprovalTable_Context", context);
			MessageHelper.hideAndClearMessage();
		},
