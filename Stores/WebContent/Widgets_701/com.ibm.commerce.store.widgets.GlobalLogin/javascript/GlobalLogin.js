...
GlobalLoginJS ={
	...
	InitHTTPSecure:function(widgetId,popup){	
		...
		if (index != -1){
			...
			popup = (typeof popup !== 'undefined')?popup:'login';
			var qPosition = newHref.indexOf('?');
			if(qPosition!==-1) {
				newHref = [newHref.slice(0, qPosition+1), 'popup='+popup+'&', newHref.slice(qPosition+1)].join('');
			} else {
				newHref += '?popup='+popup;
			}
	...
	deleteUserLogonIdCookie:function(contractURL){
		...
		var persistentCookie = getCookie("WC_PERSISTENT");
		if( persistentCookie != null){
			dojo.cookie("WC_PERSISTENT", null, {expires:-1,path:'/'});
		}	
		/*var pincodeCookie = getCookie("DMART_Pincode_Cookie");
		if( pincodeCookie != null){
			dojo.cookie("DMART_Pincode_Cookie", null, {expires:-1,path:'/'});
			document.getElementById("delivery_pin").innerHTML="";
		}*/
		
		var orderCookie = getCookie("DM_OrderId");
		if(orderCookie != null){
			dojo.cookie("DM_OrderId", null, {expires:-1,path:'/'});
		}
		
		var cartFlagCookie = getCookie("CartFlag");
		if(cartFlagCookie != null){
			dojo.cookie("CartFlag", null, {expires:-1,path:'/'});
		}
	...
	submitGLSignInForm:function(formId, widgetId) {				
		...
		if (document.getElementById(widgetId+ "_logonErrorMessage_GL" ) != null){		
			document.getElementById(widgetId+ "_logonErrorMessage_GL" ).innerHTML="";
		}
		//handle redirection after login - AE-2265 - BEGIN
		
		var returnURL = $('#'+formId+' #returnURL').val();
		var currentURL= window.location.href;
		// Fix for JIRA 6439
		params["prevPage"] = document.getElementById(formId).previousPage.value;
		var prevPage = document.getElementById(formId).previousPage.value;
		
		//URL is set as the previous page url if user is not using the header drop down for Login 
		if(prevPage != null && prevPage.length > 0 && currentURL.indexOf('DMartLogonView') != -1){
			currentURL = prevPage;
		} // End JIRA 6439
		if(returnURL === undefined || returnURL==""){
			currentURL=currentURL.replace(/#_/g,'');
			currentURL=(currentURL.indexOf("DMartLogonView")>-1||currentURL.indexOf("UserRegistrationForm")>-1)?window.location.protocol+"//"+window.location.hostname:currentURL;
			
			$('#'+formId+' #returnURL').val(currentURL);
		}
	...
	/**
	 * method to set cookie for social sign in redirect. 
	 * In case of social sign in redirection happens through FB/ GP
	 * uses cookie to redirect user to checkout page if required
	 */
	setSocialLogonCookie: function(isCheckout){
		var CookieDate = new Date;
    	CookieDate.setFullYear(CookieDate.getFullYear()+1);
    	document.cookie = "socialCheckout=" + isCheckout + ";expires=" + CookieDate.toGMTString( ) + ";path=/";
	},
