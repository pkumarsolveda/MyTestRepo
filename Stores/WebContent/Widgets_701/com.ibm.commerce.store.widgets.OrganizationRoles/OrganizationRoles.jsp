...
<c:choose>
	...
	<c:when test="${canAdministerRolesForThisOrg eq 'false'}">
		<fmt:message bundle="${widgetText}" key="ORG_NO_AUTH_TO_MANAGE_ROLE" var="rolesErrorMessage"/>
	...
	<c:otherwise>
		...
		<c:choose>
			<c:when test="${parentOrgRolesEmpty eq 'true'}">
				<fmt:message bundle="${widgetText}" key="ORG_PARENT_ROLES_EMPTY" var="rolesErrorMessage"/>
				<%@ include file="OrganizationRoles_No_Auth_UI.jspf" %>
			</c:when>
			<c:otherwise>
