...
<script type="text/javascript" src="${staticAssetContextRoot}${env_siteWidgetsDir}com.ibm.commerce.store.widgets.OrganizationUsersList/javascript/OrganizationUsersList.js"></script>
<script type="text/javascript" src="${staticAssetContextRoot}${env_siteWidgetsDir}com.ibm.commerce.store.widgets.OrganizationUsersList/javascript/OrganizationUsersListControllers.js"></script>
<script type="text/javascript">
	require(["dojo/ready", "dojo/topic"], function(ready, topic){
		ready(function(){
			...
			OrganizationUsersListJS.subscribeToOrgChange('${requestScope.orgEntityId}');
...
<div id="<c:out value='${widgetName}'/>_Widget" >
	...
	<span id="WC_${widgetName}_OrganizationUserList_ToolTip" tabindex="0" class="more_info_icon verticalAlign_middle">
		<img class="info_on" src="${env_imageContextPath}/Widgets_701/images/icon_info_ON.png" alt="<fmt:message key="${widgetNameCaps}_TTITLE_TOOLTIP" bundle="${widgetText}"/>"/>
		<img class="info_off" src="${env_imageContextPath}/Widgets_701/images/icon_info.png" alt="<fmt:message key="${widgetNameCaps}_TTITLE_TOOLTIP" bundle="${widgetText}"/>"/>
	</span>
	</div>
	<span class="spanacce" id="<c:out value='${widgetName}'/>_widget_ACCE_Label" aria-hidden="true"><fmt:message key="ACCE_REGION_${widgetNameCaps}" bundle="${widgetText}" /></span>
	<div dojoType="wc.widget.RefreshArea" widgetId="<c:out value='${widgetName}'/>Table_Widget" id="<c:out value='${widgetName}'/>Table_Widget" controllerId="<c:out value='${widgetName}'/>Table_controller" role="region" aria-labelledby="<c:out value='${widgetName}'/>_widget_ACCE_Label" ariaLiveId="${ariaMessageNode}" ariaMessage="<fmt:message key="ACCE_STATUS_${widgetNameCaps}_UPDATED" bundle="${widgetText}" />">
		<c:choose>
			<c:when test="${empty requestScope.orgEntityId}">
				<script type="text/javascript" src="${staticAssetContextRoot}${env_siteWidgetsDir}Common/MyAccountList/javascript/MyAccountList.js"></script>
			</c:when>
			<c:otherwise>
	...
	<!-- 							</div> -->
								<div class="body" id="WC_${widgetName}_OrganizationUserList_ToolTip_div">
									<fmt:message key="${widgetNameCaps}_TTITLE_TOOLTIP" bundle="${widgetText}"/>
								</div>
							</div>
							<div class="clear_float"></div>
						</div>
